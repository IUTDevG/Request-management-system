    ...
    <x-slot name="trigger">
        ...
        >
            @if ($isFlagsOnly || $hasFlags)
                ...
                />
            @else
                <span class="font-semibold text-md text-gray-700 dark:text-gray-200">{{ $languageSwitch->getCharAvatar(app()->getLocale()) }}</span>
            @endif
        </div>
    </x-slot>
        ...
        @foreach ($locales as $locale)
            @if (!app()->isLocale($locale))
                <button
                    ...
                    @class([
                        'flex items-center w-24 transition-colors duration-75 rounded-md outline-none fi-dropdown-list-item whitespace-nowrap disabled:pointer-events-none disabled:opacity-70 fi-dropdown-list-item-color-gray hover:bg-gray-950/5 focus:bg-gray-950/5 dark:hover:bg-white/5 dark:focus:bg-white/5',
